...
public class ILP extends Abstract_BSM_Algorithm
{
	...
	public Marriage match()
	{
		...
		try
		{
			...
			int[][] finalMatching = new int[finalMatchingList.size()][2];
            for (int i = 0; i < finalMatchingList.size(); i++) {
                ArrayList<Integer> finalMatch = finalMatchingList.get(i);
                finalMatching[i][0] = finalMatch.get(0);
                finalMatching[i][1] = finalMatch.get(1);
            }
			...
			this.computeFairnessMetrics(finalMatchingList);

			return new Marriage(this.n, finalMatching);
		} catch (GRBException e)
		{
			System.out.println("Error code: " + e.getErrorCode() + ". " + e.getMessage());
		}

		return new Marriage(0, new int[0][2]);
